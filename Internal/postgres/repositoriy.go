...
func (r *Repository) Insert_Content(content structures.ContentDTO) (int, error) {
	...
	err = r.client.QueryRow(context.Background(), query, content.Content).Scan(&i)
	config.Err_log(err)
	return i, err
}
...
query := "SELECT http_string.id,http_string.content,author.id,author.name FROM http_string inner join author on http_string.author_id = author_id"
